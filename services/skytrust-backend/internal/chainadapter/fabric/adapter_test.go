...
func (r *recTransport) SubmitTx(ctx context.Context, contract, method string, params map[string]any) (*chainadapter.TxReceipt, error) {
	...
	return &chainadapter.TxReceipt{TxID: "FABRIC-T1", Status: 0}, nil
}
...
func TestAdapterChainNameAndDelegation(t *testing.T) {
	...
	if a.ChainName() != "fabric" {
		t.Fatalf("chain name: %s", a.ChainName())
	}
	rc, err := a.SubmitTx(context.Background(), "operator_business", "RegisterReceive", map[string]any{"k": "v"})
	if err != nil || rc.TxID != "FABRIC-T1" || tr.calls != 1 || tr.method != "RegisterReceive" {
...
func TestAdapterRejectsUnregisteredContract(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "not registered") || !strings.Contains(err.Error(), "contracts/fabric/") {
...
func TestRegisteredContractsMatchCodifiedConstants(t *testing.T) {
	for _, c := range []string{"operator_business"} {
