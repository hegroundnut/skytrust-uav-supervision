...
func (r *recTransport) SubmitTx(ctx context.Context, contract, method string, params map[string]any) (*chainadapter.TxReceipt, error) {
	...
	return &chainadapter.TxReceipt{TxID: "FISCO-BCOS-T1", Status: 0}, nil
}
...
func TestAdapterChainNameAndDelegation(t *testing.T) {
	...
	if a.ChainName() != "fisco-bcos" {
		t.Fatalf("chain name: %s", a.ChainName())
	}
	rc, err := a.SubmitTx(context.Background(), "uav_management", "RegisterReceive", map[string]any{"k": "v"})
	if err != nil || rc.TxID != "FISCO-BCOS-T1" || tr.calls != 1 || tr.method != "RegisterReceive" {
...
func TestAdapterRejectsUnregisteredContract(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "not registered") || !strings.Contains(err.Error(), "contracts/fisco/") {
...
func TestRegisteredContractsMatchCodifiedConstants(t *testing.T) {
	for _, c := range []string{"uav_management"} {
